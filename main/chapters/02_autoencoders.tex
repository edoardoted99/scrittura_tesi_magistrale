...
fondamentali dell’originale. Gli autoencoder incarnano esattamente questo principio. Sono
...
dall’input a uno spazio latente e, a partire da tale spazio, una seconda
mappatura che riporta ai dati originali. L’addestramento è guidato
dall’obiettivo di minimizzare un errore di ricostruzione, cioè la
...
Un autoencoder è un modello di apprendimento il cui obiettivo è
...
un encoder e un decoder. L’encoder è una funzione
...
bias induttivo, ossia un insieme di vincoli che limitino la capacità
...
La prima agisce sull’architettura, riducendo il canale informativo tra
...
sulla funzione obiettivo, aggiungendo termini di regolarizzazione che
...
imporre proprietà come norma controllata o sparsità delle attivazioni,
...
\begin{enumerate}[label=\roman*.]
    \item Penalizzazione $\ell_2$ (codifiche a norma controllata).
    ...
    \item Penalizzazione $\ell_1$ (sparsità).
    ...
    \item Vincolo \textit{top-$k$} (sparsità esplicita).
...
Per ottenere queste proprietà desiderabili, è necessario introdurre vincoli espliciti nella funzione obiettivo o nell'architettura del modello. Questi vincoli agiscono come ``leggi fisiche'' che governano lo spazio latente, forzando l'emergere di una struttura ordinata e interpretabile.
...
In un autoencoder standard, questi fattori vengono tipicamente codificati in modo entangled: modificare una singola coordinata latente $z_i$ può simultaneamente alterare l'identità della cifra, la sua inclinazione e lo spessore, rendendo impossibile un controllo selettivo degli attributi.
...
\begin{enumerate}[label=\roman*.]
    \item Controllo fine: è possibile manipolare selettivamente attributi specifici;
    \item Interpretabilità: ogni dimensione latente ha un significato semantico chiaro;
    \item Trasferibilità: le feature apprese possono essere riutilizzate in compiti diversi;
    \item Robustezza: il modello generalizza meglio a combinazioni non viste di attributi.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
    \item Il solo obiettivo di ricostruzione è invariante per trasformazioni dello spazio latente: qualsiasi permutazione o rotazione delle coordinate latenti che preservi la capacità ricostruttiva è ugualmente valida;
    \item Il modello tende a sfruttare correlazioni spurie nei dati di training, mescolando fattori che co-occorrono frequentemente (es. cifre sottili tendono a essere inclinate);
    \item In assenza di vincoli espliciti, il modello converge a soluzioni localmente ottime ma semanticamente opache, dove l'informazione è distribuita in modo denso su tutte le dimensioni.
\end{enumerate}
...
Quest'ultima strategia—la sparsità—si rivela particolarmente efficace e costituisce la base degli \textit{Sparse Autoencoders}, che verranno discussi in dettaglio nel seguito. Prima, però, è utile esaminare un esempio paradigmatico di come i vincoli probabilistici possano indurre struttura interpretabile: i $\beta$-VAE.
...
Abbiamo stabilito che il disentanglement richiede vincoli espliciti sulla struttura delle rappresentazioni latenti. In questa sezione esaminiamo un approccio paradigmatico a questo problema: i Variational Autoencoders (VAE) e la loro variante $\beta$-VAE, che implementano vincoli di natura probabilistica per indurre ordine e interpretabilità nello spazio latente.
