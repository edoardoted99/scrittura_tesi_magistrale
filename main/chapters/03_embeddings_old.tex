...
\subsection{Simboli e significati}
Il punto di partenza per comprendere gli \textit{embeddings} risiede nella distinzione tra \textbf{significante} e \textbf{significato}. In linguistica, il significante è la forma fisica del segno (il suono della parola o la stringa di caratteri ``mela"), mentre il significato è il concetto mentale a cui esso rimanda. Le macchine operano nativamente nel dominio dei significanti: esse manipolano simboli discreti che, per loro natura, sono arbitrari. Non vi è nulla nella stringa ``cane" che richiami le proprietà biologiche dell'animale. La sfida dell'Intelligenza Artificiale è costruire una relazione, o funzione di mapping, che proietti l'insieme dei simboli in uno spazio semantico dove i concetti sono rappresentati numericamente.
...
\begin{tikzpicture}[scale=0.8]
    ...
    \filldraw (3.5,1) circle (2pt) node[anchor=west] (v1) {$\vec{v}_{canine}$};
    \filldraw (3.5,-1.5) circle (2pt) node[anchor=west] (v2) {$\vec{v}_{medical}$};
...
\end{tikzpicture}
\caption{Rappresentazione del mapping tra lo spazio discreto dei simboli (Significanti) e lo spazio continuo dei vettori (Significati). 
L'obiettivo è apprendere una funzione $f$ tale che simboli diversi con significati simili vengano proiettati in vettori vicini nello spazio matematico.}
\end{figure}
...
\subsection{Gli assi del linguaggio}
Per addestrare questi modelli a catturare i significati dai significanti, li si pone nella condizione di analizzare enormi corpora di testo attraverso le due dimensioni fondamentali lungo le quali si articola ogni lingua: gli \textbf{assi saussuriani}.
\begin{figure}[htbp]
    \centering
    \tikzset{
...
Per costruire queste relazioni di singificato è necessario partire da un'ipotesi fondamentale. Supponiamo di non conoscere il significato della parola \textit{ongchoi}, ma di 
...
verdura a foglia simile a queste altre verdure.
...
lavoro di Osgood \parencite{Osgood1957Measurement}, che studiarono la componente affettiva delle parole. 
...
\subsection{Dal sintagma al paradigma: finestra di contesto e precisione semantica}
L’apprendimento degli embeddings non produce semplici liste di numeri, ma uno \textbf{spazio latente}: un costrutto geometrico in cui le parole sono punti e la vicinanza tra punti approssima una vicinanza di significato. L’idea chiave è che questo spazio non viene imposto a priori, ma \emph{emerge} dai dati testuali: il modello osserva la catena \textbf{sintagmatica} (le combinazioni effettive di parole nel testo) e, da essa, ricostruisce un ordine \textbf{paradigmatico} (somiglianza e sostituibilità). In altri termini, il sintagma fornisce l’evidenza empirica; il paradigma è la struttura semantica che ne risulta.

\subsubsection{Il sintagma come architetto del paradigma}
La tesi che guiderà questa sezione può essere formulata in modo semplice:

\begin{center}
\emph{Sintagma più ricco} $\;\Rightarrow\;$ \emph{Paradigma più ristretto} $\;\Rightarrow\;$ \emph{Rappresentazione più precisa}.
\end{center}

In linguistica, più informazioni possediamo sulle parole che circondano un termine, più si restringe l’insieme di alternative plausibili che potrebbero sostituirlo senza rompere la coerenza della frase. Consideriamo una parola ignota $X$ in due contesti con diverso grado di informazione:

\begin{enumerate}
    \item \textit{Il [X] mangia.}
    \item \textit{Il [X] fedele mangia l'osso in giardino.}
\end{enumerate}

Nel primo caso, il sintagma è povero: la posizione $[X]$ può essere riempita da un insieme vastissimo di candidati (\textit{cane}, \textit{gatto}, \textit{bambino}, \textit{uomo}, \textit{animale}, \dots). Nel secondo caso, il sintagma diventa informativo: aggettivi come \textit{fedele} e oggetti come \textit{osso}, insieme allo scenario \textit{in giardino}, restringono drasticamente le sostituzioni plausibili, facendo emergere un candidato paradigmatico molto più specifico (in modo prototipico, \textit{cane}). 
La conseguenza concettuale è diretta: \textbf{arricchire il sintagma} significa fornire più vincoli che \textbf{raffinano il paradigma}. Ed è esattamente questo che un modello di embedding cerca di fare: trasformare regolarità sintagmatiche osservabili in una geometria paradigmatica latente.

\begin{notebox}
\textbf{Associazione paradigmatica}\\
Si definisce associazione paradigmatica il legame tra parole interscambiabili nello stesso contesto. Negli embeddings, questo legame diventa geometrico: parole che condividono contesti (e quindi vincoli sintagmatici simili) tendono ad occupare coordinate prossime nello spazio latente.
\end{notebox}

\subsubsection{La finestra di contesto come manopola semantica in Word2Vec}
Nel caso di Word2Vec (in particolare Skip-gram), il ``pezzo'' di sintagma che il modello osserva è controllato da un iperparametro: l’ampiezza della finestra di contesto $m$. Variando $m$, decidiamo quanta informazione sintagmatica il modello può usare per inferire il paradigma.

\begin{itemize}
    \item \textbf{Finestra stretta} ($m \le 2$): il contesto è locale e cattura soprattutto regolarità \textbf{funzionali e sintattiche}. Tendono a risultare vicine parole che svolgono ruoli grammaticali simili (ad es.\ verbi transitivi tra loro, aggettivi tra loro), perché compaiono in cornici sintagmatiche analoghe.
    \item \textbf{Finestra ampia} ($m \ge 5$): il contesto include indizi più distanti e ricchi, catturando affinità \textbf{semantiche e tematiche}. Ad esempio, parole come \textit{ospedale} e \textit{medico} non sono intercambiabili sintatticamente, ma co-occorrono in ambienti discorsivi simili: una finestra più ampia rende più facile apprendere questo tipo di legami.
\end{itemize}

In questa prospettiva, aumentare $m$ equivale ad aumentare la porzione di sintagma osservabile e quindi, in media, a rendere più informativa la stima delle somiglianze paradigmatiche: \textbf{più vincoli contestuali} permettono al modello di posizionare le parole in modo più discriminante nello spazio latente, distinguendo meglio campi semantici, topic e associazioni.

\subsubsection{Geometria dell'analogia: il modello del parallelogramma}
Quando lo spazio latente è coerente, non si limita a raggruppare parole simili: tende anche a codificare \textbf{relazioni} come direzioni stabili. Questo si manifesta nel ragionamento analogico, spesso descritto tramite il cosiddetto modello del parallelogramma.
...
\subsubsection{Limiti dei modelli statici e l'esigenza di dinamismo}
\begin{notebox}
\textbf{Training vs inferenza}\\
Per \textbf{inferenza} si intende l’uso del modello \emph{dopo} l’addestramento: dati nuovi input, i parametri appresi $\theta$ restano fissi e il modello calcola solo le sue uscite. In forma compatta:
\[
\text{training:}\quad \theta^\star=\arg\min_{\theta}\, \mathcal{L}(\theta)
\qquad\qquad
\text{inferenza:}\quad \hat{y}=f_{\theta^\star}(x).
\]
Nel nostro caso, in inferenza $f_{\theta^\star}$ produce rappresentazioni (e/o predizioni) a partire da una frase, senza aggiornare i vettori appresi.
\end{notebox}
...
\begin{enumerate}
    \item \textbf{Polisemia:} una parola con più sensi (es.\ \textit{pesca} come frutto o come attività) viene compressa in un unico vettore che media usi diversi, riducendo la precisione semantica proprio dove il contesto dovrebbe aiutare.
    \item \textbf{Contesto ``dimenticato'' in inferenza:} Word2Vec usa la finestra sintagmatica per \emph{apprendere} i vettori, ma in \textbf{inferenza} l’embedding di una parola è recuperato come valore fisso dal dizionario del modello e non viene ricalcolato in funzione della frase specifica. Di conseguenza, la stessa parola mantiene lo stesso vettore anche in contesti diversi, e il contesto non può più agire da meccanismo di disambiguazione.
\end{enumerate}
Se il significato di una parola dipende sistematicamente dalla ricchezza del sintagma che la circonda, allora la rappresentazione dovrebbe essere una \textbf{funzione del contesto}, non una costante. Questa osservazione motiva il passaggio agli \textbf{embeddings dinamici (contestuali)}: invece di limitarsi a una finestra fissa e a un vettore unico per parola, essi mirano a incorporare informazione proveniente dall’intera sequenza, generando un vettore diverso per ogni occorrenza. Per ottenere questo comportamento servono architetture in grado di modellare il linguaggio come flusso e memoria, a partire dalle reti neurali ricorrenti.

\subsection{Embeddings dinamici}
La sezione precedente ha chiarito un punto chiave: se \textbf{un sintagma più ricco} rende \textbf{più preciso il paradigma}, allora una rappresentazione che \emph{non} può sfruttare il contesto quando viene applicata a una frase concreta è inevitabilmente limitata. Gli embeddings statici (come quelli appresi da Word2Vec) usano il contesto in \textbf{addestramento} per stimare i vettori, ma poi restituiscono un dizionario di rappresentazioni \textbf{inermi}: in \textbf{inferenza}, l’embedding di una parola è sempre lo stesso, indipendentemente dalle parole che la circondano.

Questo limite diventa evidente nei casi di \textbf{polisemia} e \textbf{omonimia}. Parole come \textit{spesso} (frequenza vs spessore) o \textit{capo} (indumento vs superiore) vengono collassate in un unico punto dello spazio vettoriale, che finisce per rappresentare una ``media'' tra sensi diversi. Ma la tesi sintagma--paradigma suggerisce l’opposto: il senso corretto non è una proprietà isolata del token, bensì emerge dall’interazione con la catena sintagmatica in cui quel token è inserito.

Gli \textbf{embeddings dinamici} (o \textbf{contestuali}) rispondono esattamente a questa esigenza: invece di associare un vettore a un \emph{tipo} di parola (\textit{word type}), associano un vettore a una sua \emph{occorrenza} concreta (\textit{word instance}), calcolandolo come funzione del contesto in cui appare. In questo approccio, la rappresentazione non è più un parametro statico recuperato da una tabella di lookup, ma un \textbf{risultato computazionale} prodotto dal modello mentre elabora l’intera sequenza.
...
A questo punto la domanda diventa strutturale: se aumentare il contesto rende gli embeddings più informativi, \textbf{come si cattura contesto su larga scala senza perdere informazione lungo la sequenza?} La storia recente delle architetture neurali per il linguaggio può essere letta come una sequenza di risposte progressive a questo vincolo.

Le prime \textbf{Reti Neurali Ricorrenti} offrono un meccanismo naturale: elaborare i token in ordine e mantenere uno stato nascosto che trasporta informazione dal passato. In linea di principio, questo stato dovrebbe accumulare contesto man mano che la sequenza avanza; in pratica, però, l’addestramento tramite \textit{backpropagation through time} rende difficile preservare segnali lontani a causa del \textbf{vanishing gradient}. All’aumentare della distanza temporale, i gradienti tendono ad attenuarsi e le dipendenze a lungo raggio risultano difficili da apprendere: l’obiettivo di ``allargare la finestra'' si scontra con un limite di ottimizzazione.

Le \textbf{LSTM} (e più in generale le architetture \textit{gated}) nascono come tentativo di stabilizzare questa memoria: separano il flusso informativo in canali (stato nascosto e cell state) e introducono gate che decidono cosa mantenere e cosa dimenticare. In questo modo, il modello può preservare meglio informazioni cruciali e ridurre la degradazione del segnale su sequenze lunghe, ma il contesto resta comunque mediato da una dinamica sequenziale, e la propagazione di informazione a grande distanza rimane onerosa.

Quando si passa a compiti \textit{sequence-to-sequence} (traduzione, riassunto, ecc.), l’architettura \textbf{Encoder--Decoder} prova a comprimere un’intera sequenza in un vettore di contesto. Qui il problema non è solo l’ottimizzazione: è anche \textbf{rappresentazionale}. Un singolo vettore deve contenere ``tutto'', introducendo un collo di bottiglia che penalizza soprattutto sequenze lunghe. Il passo successivo, il \textbf{meccanismo di attenzione}, affronta direttamente questo limite: invece di obbligare il decoder a usare un riassunto statico, consente di recuperare dinamicamente informazione da \emph{tutti} gli stati dell’encoder, costruendo un contesto diverso a ogni passo.

Il \textbf{Transformer} estremizza questa idea: sostituisce la ricorrenza con self-attention, cioè con un meccanismo che collega direttamente posizioni anche molto distanti e rende la computazione parallelizzabile. In questo quadro, aumentare il contesto non significa più “trasportare” informazione attraverso una catena temporale (con tutti i problemi di gradiente), ma \textbf{mettere in comunicazione} token lontani tramite pesi di attenzione appresi. È questo passaggio che rende finalmente praticabile l’obiettivo iniziale: produrre embeddings contestuali che incorporano ampie porzioni di sintagma in modo stabile.

Su questa base si collocano i modelli bidirezionali \textbf{BERT}: adottano la stessa architettura Transformer ma rinunciano al vincolo causale dell’autoregressione per costruire rappresentazioni che vedono \textbf{sia sinistra sia destra} del token. In altre parole, BERT spinge l’idea di embedding dinamico al suo estremo naturale: la rappresentazione di una parola non dipende da una finestra locale, né solo dal passato, ma dall’intera frase. In questo senso, l’evoluzione RNN $\rightarrow$ LSTM $\rightarrow$ attenzione $\rightarrow$ Transformer $\rightarrow$ BERT può essere letta come un’unica traiettoria: l’ampliamento progressivo del contesto utilizzabile e il tentativo di renderlo \emph{apprendibile} senza che la sequenza stessa diventi un ostacolo (vanishing gradient, bottleneck, vincoli di accesso all’informazione).
