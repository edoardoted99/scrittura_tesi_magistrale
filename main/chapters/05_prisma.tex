...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[right, font=\footnotesize, gray!70, align=left, rotate=-90] at (6.4, -1.8) {Capacità crescente};
...
\end{figure}
\subsubsection{Feature ricorrenti, splitting e feature nuove}
\label{subsubsec:recurrent_novel}

Per comprendere come le feature evolvono al variare della capacità del SAE, O'Neill et al.~\parencite{oneill2024disentangling} introducono due metriche quantitative che permettono di classificare ogni feature di un SAE più grande rispetto al SAE più piccolo.

\paragraph{Similarità geometrica.} Data una coppia di SAE con $n_1$ e $n_2$ feature rispettivamente (con $n_1 < n_2$), si calcola la matrice di similarità $S \in \mathbb{R}^{n_1 \times n_2}$ dove ogni elemento è la similarità del coseno tra i vettori decoder:
\begin{equation}
    S_{ij} = \frac{\mathbf{w}_i^T \mathbf{w}_j}{\|\mathbf{w}_i\| \|\mathbf{w}_j\|}
    \label{eq:feature_similarity}
\end{equation}
dove $\mathbf{w}_i$ è la colonna $i$-esima del decoder del SAE piccolo e $\mathbf{w}_j$ quella del SAE grande. Per ogni feature $j$ nel SAE grande, si identifica il \textit{nearest neighbour} nel SAE piccolo:
\begin{equation}
i^*(j) = \underset{i \in \{1,\dots,n_1\}}{\mathrm{arg\,max}} \; S_{ij}
\end{equation}
e si definisce la \textbf{similarità geometrica massima}:
\begin{equation}
    S^*_j = \max_{i} S_{ij} = S_{i^*(j), j}
\end{equation}

\paragraph{Similarità di attivazione.} La similarità geometrica cattura la vicinanza nello spazio dei pesi, ma non garantisce che due feature si comportino allo stesso modo sui dati. Si introduce quindi la \textbf{similarità di attivazione} $D_{ij}$, che misura quanto due feature co-attivano sugli stessi documenti:
\begin{equation}
    D_{ij} = \frac{\sum_{k=1}^{N} B_{ik} \cdot B_{jk}}{\sqrt{\sum_k B_{ik}^2} \cdot \sqrt{\sum_k B_{jk}^2}}
    \label{eq:activation_similarity}
\end{equation}
dove $B_{ik} = h_{k,i}$ è l'attivazione della feature $i$ sul documento $k$ (zero se non attiva). In pratica, $D_{ij}$ è la correlazione tra i vettori di attivazione delle due feature sull'intero corpus.

\paragraph{Classificazione delle feature.} Combinando le due metriche, ogni feature $j$ del SAE grande può essere classificata in una delle tre categorie:

\begin{enumerate}
    \item \textbf{Feature ricorrenti}: feature che riappaiono sostanzialmente invariate nel SAE più grande. Sono caratterizzate da:
    \begin{equation}
        S^*_j > \tau_{\text{high}} \quad \text{e} \quad D_{i^*(j), j} > \tau_{\text{high}}
    \end{equation}
    dove tipicamente $\tau_{\text{high}} \approx 0.95$. Queste feature rappresentano concetti ``atomici'' che non si scompongono ulteriormente---ad esempio, ``O-type stars'' in astrofisica o ``Generative Adversarial Networks'' in machine learning. La loro stabilità attraverso modelli di capacità diversa suggerisce che corrispondono a concetti fondamentali del dominio.
    
    \item \textbf{Feature da splitting}: feature che emergono dalla scissione di una feature più generale. Sono caratterizzate da:
    \begin{equation}
        \tau_{\text{low}} < S^*_j < \tau_{\text{high}} \quad \text{e} \quad D_{i^*(j), j} > \tau_{\text{mod}}
    \end{equation}
    dove tipicamente $\tau_{\text{low}} \approx 0.5$ e $\tau_{\text{mod}} \approx 0.3$. La similarità geometrica moderata indica che la nuova feature punta in una direzione correlata ma distinta; la similarità di attivazione positiva indica che si attiva su un \textit{sottoinsieme} dei documenti che attivavano la feature parent. Semanticamente, rappresentano sotto-concetti più specifici.
    
    \item \textbf{Feature nuove} (\textit{novel features}): feature che non hanno corrispondenti nel SAE più piccolo. Sono caratterizzate da:
    \begin{equation}
        S^*_j < \tau_{\text{low}} \quad \text{oppure} \quad D_{i^*(j), j} < \tau_{\text{low}}
    \end{equation}
    Queste feature emergono solo quando la capacità è sufficiente a rappresentarle separatamente. Un SAE piccolo potrebbe non avere abbastanza dimensioni per distinguere ``polmonite batterica'' da ``polmonite virale'', rappresentandole con un'unica feature ``polmonite''. Un SAE più grande può permettersi questa distinzione, facendo emergere feature nuove che non hanno un corrispondente diretto nel modello più piccolo.
\end{enumerate}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=1.0]
        % Assi
        \draw[->, thick] (0, 0) -- (6, 0) node[right, font=\small] {Similarità geometrica $S^*_j$};
        \draw[->, thick] (0, 0) -- (0, 5) node[above, font=\small] {Similarità attivazione $D_{i^*,j}$};
        
        % Soglie
        \draw[dashed, gray] (2, 0) -- (2, 4.8) node[above, font=\scriptsize] {$\tau_{\text{low}}$};
        \draw[dashed, gray] (4.5, 0) -- (4.5, 4.8) node[above, font=\scriptsize] {$\tau_{\text{high}}$};
        \draw[dashed, gray] (0, 1.5) -- (5.8, 1.5);
        \node[left, font=\scriptsize] at (0, 1.5) {$\tau_{\text{low}}$};
        \draw[dashed, gray] (0, 4) -- (5.8, 4);
        \node[left, font=\scriptsize] at (0, 4) {$\tau_{\text{high}}$};
        
        % Regioni
        % Ricorrenti (alto-destra)
        \fill[green!20, opacity=0.7] (4.5, 4) rectangle (5.8, 4.8);
        \node[font=\scriptsize, green!50!black, align=center] at (5.15, 4.4) {Ricorrenti};
        
        % Splitting (centro)
        \fill[blue!20, opacity=0.7] (2, 1.5) rectangle (4.5, 4.8);
        \node[font=\scriptsize, blue!60!black, align=center] at (3.25, 3.2) {Splitting};
        
        % Novel (sinistra e basso)
        \fill[orange!20, opacity=0.7] (0, 0) rectangle (2, 4.8);
        \fill[orange!20, opacity=0.7] (2, 0) rectangle (5.8, 1.5);
        \node[font=\scriptsize, orange!60!black] at (1, 2.5) {Novel};
        \node[font=\scriptsize, orange!60!black] at (4, 0.75) {Novel};
        
        % Punti esempio
        \fill[green!60!black] (5.2, 4.5) circle (3pt);
        \fill[blue!70] (3.5, 3.8) circle (3pt);
        \fill[blue!70] (2.8, 2.5) circle (3pt);
        \fill[orange!70!black] (1.2, 3.5) circle (3pt);
        \fill[orange!70!black] (3.8, 0.8) circle (3pt);
        
    \end{tikzpicture}
    \caption{Classificazione delle feature in base a similarità geometrica ($S^*_j$) e similarità di attivazione ($D_{i^*,j}$). Le feature \textbf{ricorrenti} (verde) hanno entrambe le similarità elevate. Le feature da \textbf{splitting} (blu) hanno similarità geometrica moderata ma attivazione correlata---si attivano su un sottoinsieme dei documenti della feature parent. Le feature \textbf{nuove} (arancione) hanno bassa similarità in almeno una delle due dimensioni, indicando concetti non rappresentati nel SAE più piccolo.}
    \label{fig:feature_classification}
\end{figure}

\paragraph{Interpretazione geometrica dello splitting.} Lo splitting può essere compreso geometricamente. Se una feature $\mathbf{w}_p$ del SAE piccolo rappresenta un concetto generale (es. ``ottimizzazione''), nel SAE grande questo concetto può scindersi in $k$ feature $\mathbf{w}_{c_1}, \dots, \mathbf{w}_{c_k}$ che rappresentano sotto-concetti (es. ``SGD'', ``Adam'', ``Newton''). Geometricamente:
\begin{equation}
    \mathbf{w}_p \approx \frac{1}{k} \sum_{i=1}^{k} \mathbf{w}_{c_i} + \boldsymbol{\epsilon}
\end{equation}
dove $\boldsymbol{\epsilon}$ è un residuo. La feature parent punta approssimativamente nella direzione ``media'' dei children. Ogni child ha similarità moderata con il parent ($0.5 < S_{p,c_i} < 0.95$) ma bassa similarità tra loro ($S_{c_i, c_j} < 0.5$ per $i \neq j$), riflettendo la specializzazione semantica.

\begin{notebox}
\textbf{Splitting vs.\ feature nuove: riepilogo}\\[0.5em]
\begin{itemize}
    \item \textbf{Feature splitting}: la feature del SAE grande ha similarità \textit{moderata} ($0.5$--$0.95$) con una feature del SAE piccolo, e si attiva su un \textit{sottoinsieme} dei suoi documenti. Semanticamente, è un sotto-concetto.
    \item \textbf{Feature nuova}: la feature del SAE grande ha similarità \textit{bassa} ($< 0.5$) con tutte le feature del SAE piccolo. Il concetto non era rappresentabile con la capacità precedente.
\end{itemize}
La distinzione si basa su due metriche complementari: $S^*_j$ (geometria dei pesi) e $D_{i^*,j}$ (comportamento sui dati).
\end{notebox}
