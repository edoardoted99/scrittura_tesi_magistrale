...
Gli autoencoder sono modelli di apprendimento non supervisionato progettati per apprendere una funzione di ricostruzione dell’input. 
Essi sono caratterizzati dalla presenza di una strozzatura informativa (\textit{bottleneck}) che costringe il modello a comprimere i dati in una rappresentazione a dimensionalità inferiore, detta \textbf{rappresentazione latente}. 
L’idea alla base è quella di indurre il modello a catturare esclusivamente le caratteristiche più rilevanti dei dati, scartando ridondanze e rumore.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[draw, rectangle, minimum height=1cm, minimum width=0.8cm, fill=red!20, right=of enc] (latent) {$n < q$};
    ...
    \end{tikzpicture}
    \caption{Architettura di un autoencoder classico. Il bottleneck forza una compressione informativa che induce l’apprendimento di una rappresentazione latente compatta in uno spazio di dimensione $n<q$.}
...
strutturate e informative. In questo contesto, la funzione di costo dell’autoencoder non si limita più a
...
base degli \textit{Sparse Autoencoders}, che verranno discussi nel seguito. Un esempio comune di regolarizzazione consiste nell’introdurre un vincolo
...
codifiche più compatte e contribuendo alla stabilità del modello. Un’alternativa è rappresentata dalla regolarizzazione di tipo $\ell_1$
...
nel seguito. L’aggiunta di vincoli nella funzione obiettivo consente quindi di superare i
...
originali, mediante una trasformazione lineare delle variabili. Sia dato un dataset di $M$ osservazioni centrate
\begin{equation*}
...
corrispondente direzione $\mathbf{v}_k$. Ordinando gli autovalori in ordine decrescente
\begin{equation*}
...
\subsubsection{Caso di un autoencoder lineare}
...
associati ai $q$ maggiori autovalori. In particolare, il minimo della funzione di costo è ottenuto quando le righe di
...
$q$ componenti principali. Ne consegue che un autoencoder lineare, addestrato mediante minimizzazione
...
interpretata come un caso particolare di autoencoder lineare. \parencite{Goodfellow2016DeepLearning}

\subsubsection{Caso di un autoencoder non lineare}
...
la varianza dei dati proiettati lungo un insieme di direzioni fisse. L’autoencoder non lineare è quindi in grado di catturare strutture complesse e
...
memorizzazione non strutturata delle osservazioni. In uno scenario ideale, le componenti dello spazio latente risultano
...
anche concettualmente significativo. Tuttavia, nella pratica, l’interpretabilità delle feature latenti non è
...
\subsection{Rappresentazioni latenti disentangled}
\label{sec:disentangled_latent}

Come si possono rendere le \textit{feature} latenti davvero interpretabili, cioè
far sì che ciascuna dimensione dello spazio latente corrisponda a un attributo
umanamente riconoscibile? In un caso ideale, muoversi lungo una direzione dello
spazio latente dovrebbe produrre una variazione controllata di un singolo
\textit{fattore di variazione} (ad esempio rotazione, spessore del tratto,
posizione), lasciando invariati gli altri. Osservando però i risultati degli
autoencoder classici (Figura~\ref{fig:latent_manifold} e Figura~\ref{fig:latent_space}),
questa dinamica raramente emerge: lo spazio latente risulta spesso poco
\textit{organizzato} e non permette un controllo semantico diretto tramite la
variazione di singole coordinate. Una rappresentazione latente si dice \textit{disentangled} quando i diversi
...
variazione e rimane (approssimativamente) invariata rispetto agli altri.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{pictures/cap2/mnist_disenangled_info_gan.png}
    \caption{Immagini MNIST generate da InfoGAN con variazioni controllate di cifra, spessore e rotazione. \parencite{chen2016infoganinterpretablerepresentationlearning}}
    \label{fig:mnist_disenangled_info_gan}
\end{figure}
Operativamente, questa proprietà si manifesta nel fatto che la manipolazione di
una singola dimensione dello spazio latente produce una variazione interpretabile
e localizzata nell’output ricostruito o generato (\textit{latent traversal}),
senza influenzare gli altri attributi dell’osservazione. Le rappresentazioni
disentangled sono quindi desiderabili per l’analisi esplorativa, il controllo
generativo, la robustezza a variazioni spurie e il trasferimento di conoscenza
tra domini. Tuttavia, le rappresentazioni disentangled non emergono spontaneamente negli
autoencoder standard. Poiché l’obiettivo è esclusivamente la minimizzazione
dell’errore di ricostruzione, l’organizzazione dello spazio latente tende a
essere funzionale a tale scopo, senza alcuna pressione esplicita a separare
semanticamente l’informazione. Di conseguenza, è comune osservare rappresentazioni
\textit{entangled}, in cui più fattori di variazione risultano mescolati sulle
stesse dimensioni. Un esempio intuitivo può essere costruito su MNIST: oltre
all’identità della cifra, i dati variano per posizione, scala, rotazione e
spessore del tratto. In una rappresentazione davvero disentangled, fissata
l’identità della cifra, la variazione controllata di una sola componente latente
dovrebbe modificare, ad esempio, esclusivamente la rotazione, lasciando invariati
posizione, scala e stile. Un’illustrazione di questo comportamento (in un modello
generativo che introduce variabili latenti interpretabili) è mostrata in
Figura~\ref{fig:mnist_disenangled_info_gan}. Per ottenere disentanglement, in generale, è necessario introdurre \textbf{vincoli}
(\textit{constraints}) o \textbf{regolarizzazioni} sullo spazio latente, in modo da
guidare l’apprendimento verso codifiche più strutturate. Una motivazione pratica
riguarda anche la geometria dello spazio latente: negli autoencoder standard la
distribuzione delle codifiche può assumere forme e orientamenti diversi tra training,
rendendo difficile sia il campionamento sia l’interpretazione; nei VAE, invece,
l’imposizione di un prior (tipicamente una gaussiana isotropa) regolarizza lo
spazio latente rendendolo più campionabile e più stabile
\parencite{chen2016infoganinterpretablerepresentationlearning}. Questa differenza
qualitativa è illustrata in Figura~\ref{fig:ae_vs_vae_latent}.
\begin{figure}[htbp]
    \centering
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{pictures/cap2/latent_ae.png}
        \caption{Spazio latente appreso da un autoencoder classico: geometria
        irregolare e sensibile al training.}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{Spazio latente appreso da un Variational Autoencoder: la
        regolarizzazione verso un prior noto induce una distribuzione più compatta
        e campionabile.}
        \label{fig:vae_latent_stability}
    \end{subfigure}

    \caption{Confronto tra spazi latenti appresi su MNIST.
    (\subref{fig:ae_latent_instability}) Autoencoder classico.
    (\subref{fig:vae_latent_stability}) Variational Autoencoder.
    Figure da \parencite{gordon2020machine}.}
    \label{fig:ae_vs_vae_latent}
\end{figure}
Una delle formulazioni più note che esplicita l’idea di ``più vincolo sul latente''
è la \textbf{$\beta$-VAE}, che modifica l’obiettivo variazionale introducendo un
peso $\beta$ sul termine di regolarizzazione. Indicato con $q_\phi(\mathbf{z}|\mathbf{x})$
l’encoder variazionale (approssimazione del posteriore), con $p_\theta(\mathbf{x}|\mathbf{z})$
il decoder generativo e con $p(\mathbf{z})$ il prior, l’ELBO standard è:
\begin{equation}
...
\end{equation}
Nella $\beta$-VAE si introduce:
\begin{equation}
\mathcal{L}_{\beta\text{-VAE}}(\theta,\phi;\mathbf{x})
=
\mathbb{E}_{q_\phi(\mathbf{z}|\mathbf{x})}\!\left[\log p_\theta(\mathbf{x}|\mathbf{z})\right]
-
...
\label{eq:beta_vae}
\end{equation}
Aumentare $\beta$ rafforza la pressione affinché le codifiche latenti rispettino
il prior (tipicamente fattorizzato), favorendo una maggiore indipendenza tra
componenti e quindi \textit{potenzialmente} un migliore disentanglement.
Questo beneficio viene però pagato con un trade-off: una regolarizzazione troppo
forte può peggiorare la qualità della ricostruzione, poiché riduce la capacità
informativa del latente. In pratica, la scelta di $\beta$ bilancia quindi
\textit{fedeltà di ricostruzione} e \textit{struttura/interpretabilità} del latente.
In sintesi, per ottenere rappresentazioni disentangled è necessario introdurre
vincoli che \textbf{regolarizzino} lo spazio latente e riducano la codifica
diffusa dell’informazione, promuovendo separazione e (almeno approssimativamente)
indipendenza tra fattori. Questa idea si collega naturalmente a quanto discusso
nel seguito: l’imposizione di \textbf{sparsità} (Sparse Autoencoders) rappresenta
un’ulteriore forma di vincolo strutturale che può favorire l’emergere di feature
più selettive e spesso più interpretabili.

% BibTeX consigliato (se usi biblatex):
...
% }
%
% @misc{rajaa2022featuredisentanglement,
%   author       = {Shangeth Rajaa},
%   title        = {Feature Disentanglement - I},
%   howpublished = {Skit Tech Blog},
%   year         = {2022},
%   url          = {https://tech.skit.ai/feature-disentanglement1/},
%   note         = {Ultimo accesso: YYYY-MM-DD}
% }
...
questa impostazione prendono il nome di \textbf{Sparse Autoencoders}. A differenza degli autoencoder classici con strozzatura (bottleneck), nei quali la
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale=0.6, transform shape, node distance=1cm]
        ...
        % Sparse Latent
        \node[draw, rectangle, minimum height=4cm, minimum width=0.5cm, fill=red!20, label=above: spazio latente $\mathbf{h}$] (latent) at (6,0) {$h \in \mathbb{R}^{q}$};
        ...
        \draw[->, thick] (dec) -- (output);
    \end{tikzpicture}
    \caption{Architettura di un autoencoder con dimensionalitàl latente $q \leq n$ }
\end{figure}
\begin{figure}[htbp] 
    \centering 
    \includegraphics[width=\linewidth]{pictures/SAE_diagram.png} 
...
interpretabili. Formalmente, dati un encoder $g_\theta$ e un decoder $f_\phi$, la funzione obiettivo
...
vincoli di sparsità sulle attivazioni latenti. Una delle scelte più comuni consiste nell’applicare una penalizzazione di tipo
...
indipendente dalla scala delle attivazioni. Sebbene la sparsità non garantisca in senso rigoroso una completa separazione
