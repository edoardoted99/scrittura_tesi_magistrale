...
L’uso degli embedding offre due grandi vantaggi:

\begin{itemize}
    \item \textbf{Generalizzazione}: parole simili hanno embedding simili, quindi il 
    modello può generalizzare a contesti mai visti (es.\ ``the dog gets'' → ``fed'').
    \item \textbf{Efficienza}: gli embedding riducono drasticamente la dimensionalità e 
    permettono economie computazionali rispetto ai modelli basati su one-hot.
\end{itemize}

Questi concetti costituiscono la base dei moderni modelli linguistici neurali: reti 
capaci di apprendere automaticamente rappresentazioni sempre più ricche del 
